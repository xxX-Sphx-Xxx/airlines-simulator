...
#Client web
$cw  = new-object System.Net.WebClient
...
$download_dir = "$project_dir"+"\apps_download\"
...
Write-Warning "Le projet sera installÃ© a la racine du lecteur $lecteur" -WarningAction Inquire
#CrÃ©ation du dossier de téléchargement des applications
...
$cw.DownloadFile($python_url,$download_dir+$python_exe)
#curl -Uri $python_url
Write-Host "télé©chargement et installation de Wamp $osarchitecture"
if ($osarchitecture -eq "64 bits"){
    ...
    $wamp_exe = $wamp32_exe}
$cw.DownloadFile($wamp_url,$download_dir+$wamp_exe)
...
if ($LASTEXITCODE -ne 0){
    throw "L'installation de $python_exe c'est terminÃ© avec l'erreur : '$LASTEXITCODE'"}
...
#TÃ©lÃ©chargement de wamp
Write-Host "tÃ©lÃ©chargement et installation de Wamp $osarchitecture"
if ($osarchitecture -eq "64 Bits"){
    $wamp_url = wamp64_url
    $wamp_install_dir = wamp64_install_dir
    $wamp_exe = wamp64_exe}
...
if ($LASTEXITCODE -ne 0){
    throw "L'installation de wamp_exe c'est terminÃ© avec l'erreur : '$LASTEXITCODE'"}
...
Write-Host @"
Pour installer la base de donnÃ©e il faut vous connecter sur localhost:8080 et importer le fichier suivant:
- projects\Airlines\structure_db pour importer uniquement la structure des tables
- projects\Airlines\structure_datas_db pour importer la structure des tables et les donnÃ©es de rÃ©fÃ©rence
...
Write-Host "Source Ã  l'adresse suivante : "
